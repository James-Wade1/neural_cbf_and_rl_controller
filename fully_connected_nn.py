...
class FullyConnectedCBF(nn.Module):
...
if __name__ == "__main__":
    ...
    model = FullyConnectedCBF(input_size=6, hidden_size=128, num_layers=2).to(device)
    ...
    train_loss1, val_loss1 = train_neural_cbf(model, train_loader, val_loader, optimizer, criterion, device, num_epochs=50)

    from resnet import ResNetCBF, ResidualBlock  # To avoid circular imports
    ...
    train_loader = DataLoader(train_dataset, batch_size=64, shuffle=True)
    val_loader = DataLoader(val_dataset, batch_size=64, shuffle=False)
    train_loss2, val_loss2 = train_neural_cbf(model, train_loader, val_loader, optimizer, criterion, device, num_epochs=50)

    plt.plot(train_loss1, label='Train Loss Fully Connected')
    plt.plot(val_loss1, label='Validation Loss Fully Connected')
    plt.plot(train_loss2, label='Train Loss ResNet')
    plt.plot(val_loss2, label='Validation Loss ResNet')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.yscale('log')
    plt.title('Training and Validation Loss for Learned CBF Models')
    plt.legend()
    plt.show()

    model.eval()
    # torch.save(model, "final_project_data/fully_connected_cbf_model.pth")